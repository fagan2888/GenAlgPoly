...
%\acrodef{GAPolyreg}[\textsc{GAPoly.reg}]{Genetic Algorithms for Polynomials with Regularization}

\acrodef{GAPoly}[EPR]{Evolutionary Polynomial Regression}
\acrodef{GAPolyreg}[EPR$^2$]{Evolutionary Polynomial Regression with Regularisation}
...
\Function{EPR}{$D,pop_0,\epsilon$}
	...
	\While{$err > \epsilon \land \neg\Call{limit}{iterations}$}
...
This \ac{rmse} function tends to prefer more complex polynomials, namely in the number of monomials which provides the regression algorithm for more fitting possibilities. One way to balance this is to provide a regularization term into the \ac{rmse} function. The article's proposal is to include a multiplicative factor proportional to the number of monomials. Thus, the error from equation \ref{eq:rmse} defines
\begin{eqnarray}
...
In the next datasets, the population of the genetic algorithm had size~$n=300$ with 100~iterations for each run. \ac{GAPolyreg} was run with regularization parameter $lambda = 0.8$.
