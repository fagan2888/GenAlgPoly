...
\end{enumerate}
This encoding can also be viewed as the flattening of the binary exponents in the matrix representation prefixed by the activation segment. The set $\left\lbrace x_1^3 x_3, x_3^7, x_1 x_2\right\rbrace$ (where $m = 3$) has matrix representation
$$\left[ 
\begin{array}{ccc}
3 & 0 & 1 \\
0 & 0 & 7 \\
...
$$
where the right matrix is in binary form with  $d = 3$ bits. An encoding where the extra monomial $x_1^6x_2^2x_3^5$ is inactive, setting $k = 4$, would be
\begin{center}
...
A known problem of using a cost function based only in the dataset \ac{rmse} and with polynomial regressions in general is a tendency to overfit the training data. The excessive variance is can be reduced by a regularization of the \ac{rmse} function. Tnd the inclusion of a penalty factor proportional to the number of monomials is a possible regularization method. Thus, the error from equation \ref{eq:rmse} defines
\begin{eqnarray}
...
\caption{Error distribution by regularization exponent for the Abalone and Auto MPG datasets. The box plots summarize the error values of 10 runs for each value of $\lambda$. The smallest overall error is shown in grey and, in both cases, was achieved with $\lambda=0.8$.}
