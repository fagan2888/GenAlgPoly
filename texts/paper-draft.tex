...
%
\begin{eqnarray}
J_{fit}\left(\Theta;\mathcal{M},\mathcal{D}\right) &=& \sqrt{\frac{1}{N}\sum_{i=1}^N \left( y\sample{i} - h_{\Theta,\mathcal{M}}\left( x_1\sample{i},\ldots,x_n\sample{i} \right) \right)^2 }\label{eq:rmse}
\end{eqnarray}
%
...
\begin{eqnarray}
J_{reg}\left(\Theta;\mathcal{M},\mathcal{D}\right) &=& \lambda^{m} J_{fit}\left(\Theta;\mathcal{M},\mathcal{D}\right)\label{eq:rmse-reg}
\end{eqnarray}
%
...
\begin{axis}[width=0.90\textwidth,height=6cm,
	...
	xmin=-5,
	xmax=300,
	xlabel={Error distribution (\ac{rmse})},
...
\begin{tikzpicture}
\begin{axis}[width=0.98\textwidth,height=6cm,
...
\begin{figure}[htb]\begin{center}
\begin{tikzpicture}
\begin{axis}[width=0.98\textwidth,height=7cm,
	boxplot/draw direction=y,
	xtick={1,2,3,4,5,6},
	xticklabels={GA.Poly,Lin.Reg,SVM,RPART,Rnd.For,CI.Tree},
	xlabel={Regression Method},
...
The authors thank the Fundação para a Ciência e Tecnologia (FCT) and the  laboratório de I\&D LabMAg the financial support given to this work under the strategic project PEst-­OE/EEI/UI0434/2011.
