...
save(report, file="Housing300_lambda0.8.Rda") # recover with load("Housing300.Rda") 
...
write.table(df, "Housing300_lambda0.8.txt")
...
source("gapolyfuncs.R")
report <- test.regularization(my.data, population=150, iterations=75, lambda=0.8, runs=50)

save(report, file="Housing300_lambda0.8.vs.no.reg_pop200.Rda") # recover with load("Housing300.Rda") 

df <- data.frame(GA.Poly = report$ga.error,
                 GA.Poly.reg = report$ga.reg.error)

write.table(df, "Housing300_lambda0.8.vs.no.reg_pop200.txt")
...
source("gapolyfuncs.R")
errors <- test.lambda(my.data, lambda.values, runs=n.runs, population=5, iterations=15)
